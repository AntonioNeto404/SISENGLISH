...
<div class="container mt-4">
    <h2><i class="fas fa-user-graduate"></i> Certidão de Discentes</h2>
    <hr>
    <form method="get" class="form-inline mb-3 d-print-none">
        <label class="mr-2">Discente:</label>
...
<?php if($selected): ?>
    ...
    $estudanteObj->id = $selected;
    if(!$estudanteObj->readOne()) {
        echo '<div class="alert alert-warning">Estudante não encontrado ou não existem registros de discência.</div>';
    } else {
    $mat = new Matricula($db);
    $mat->estudante_id = $selected;
    $stmtC = $mat->getCoursesByStudent();
    ?>    <div id="certidaoDiscencia" class="certificate" style="width: 100%; max-width: 800px; margin: 0 auto; padding: 20px; border: 1px solid #000; page-break-after: always; background-color: white; font-family: Arial, sans-serif;">
        <!-- Cabeçalho com logo -->
        ...
        <h1 style="text-align: center; font-size: 18px; font-weight: normal; margin: 30px 0 20px 0;">C E R T I D Ã O &nbsp; D E &nbsp; D I S C Ê N C I A</h1>
          <p style="text-align: justify; font-size: 12px; margin-bottom: 30px; line-height: 1.5;">
            ...
            Defesa Social - ACIDES, o servidor abaixo descrito participou na qualidade de Discente 
        ...
        <div style="margin-bottom: 30px; font-size: 12px;">
            <table width="100%" style="border-collapse: collapse;">
                <tr>
                    ...
                    <td width="80%" style="padding: 5px 0;"><?php echo htmlspecialchars(strtoupper($estudanteObj->nome)); ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold; padding: 5px 0;">Instituição:</td>
                    <td style="padding: 5px 0;"><?php echo htmlspecialchars(strtoupper($estudanteObj->forca)); ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold; padding: 5px 0;">Matr.:</td>
                    <td style="padding: 5px 0;"><?php echo htmlspecialchars($estudanteObj->matricula); ?></td>
                </tr>
            </table>
        </div>
          <!-- Cursos -->
            ...
            while($c = $stmtC->fetch(PDO::FETCH_ASSOC)):
                ...
                if($c['ano'] != $prevAno) {
                    ...
                    echo '<tr style="background-color: #f8f8f8;">
                        <th style="text-align: left; padding: 5px; border: 1px solid #ddd;">Curso</th>
                        ...
                        <th style="text-align: center; width: 60px; border: 1px solid #ddd;">Port. Conc.</th>
                    </tr>';
                    $prevAno = $c['ano'];
                }
            ?>                <tr>
                    <td style="padding: 5px; border: 1px solid #ddd;"><?php echo htmlspecialchars($c['curso'] ?? '-'); ?></td>
                    ...
                    <td style="text-align: center; border: 1px solid #ddd;"><?php echo htmlspecialchars($c['autorização'] ?? ''); ?></td>
                </tr>
            <?php endwhile; ?>
            <?php if(!$primeiro): ?></table><?php endif; ?>
        </div>        <!-- Assinatura -->
        <div style="margin-top: 80px; display: flex; justify-content: center;">            <div style="text-align: center;">
    ...
    function gerarPDF() {
        ...
        var element = document.getElementById('certidaoDiscencia');
        var opt = {
            margin: 10,
            filename: 'certidao_discencia_<?php echo $estudanteObj->matricula; ?>.pdf',
...
<?php } // Fechamento do else (estudante encontrado) ?>
<?php endif; ?>
</div>
